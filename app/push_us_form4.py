#!/usr/bin/env python3
"""从 data/form4/ 目录读取 Form 4 JSON，增量写入 Supabase。

Pipeline:
  1. 扫描文件 → 与 manifest 对比 → 找出变更文件
  2. 按 ticker 分组 → 并行处理（ticker 级并行）
  3. 批量写入 filings + transactions
  4. 保存 manifest
"""
...
from lib.db import upsert_form4_filings, upsert_form4_transactions
...
def scan_all_files(data_dir: Path, tickers: set[str] | None = None) -> list[Path]:
    """扫描 data/form4/tickers/[ticker]/*.json 文件。"""
...
def process_ticker(
    ...
    """读取该 ticker 的所有变更文件 → 提取 filings + transactions。"""
    payload = TickerPayload(ticker=ticker)

    for file_path in files:
        try:
            data = json.loads(file_path.read_text())
            filing_row = data.get("filing", {})
            if filing_row:
                payload.filings.append(filing_row)
            txn_rows = data.get("transactions", [])
            payload.transactions.extend(txn_rows)
    ...
    if payload.filings:
        try:
            upsert_form4_filings(payload.filings)
        except Exception as e:
            payload.errors.append(f"filings upsert: {e}")
            payload.processed_files.clear()

    if payload.transactions:
        try:
            upsert_form4_transactions(payload.transactions)
        except Exception as e:
            payload.errors.append(f"transactions upsert: {e}")
            if not payload.errors[:-1]:  # 如果 filings 没报错
                payload.processed_files.clear()
...
def run(args: argparse.Namespace) -> None:
    ...
    for ticker, files in all_ticker_files.items():
        ticker_dir = data_dir / "tickers" / ticker
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            if payload.processed_files:
                ...
                total_filings += len(payload.filings)
            ...
            if not args.quiet:
                ...
                console.print(
                    f"  {ticker}: f={len(payload.filings)} t={len(payload.transactions)} {status}"
                )
    ...
    summary.add_row("写入 filings", f"{total_filings:,}")
    summary.add_row("写入 transactions", f"{total_transactions:,}")
    ...
    if args.quiet:
        console.print(
            ...
            f"f={total_filings} t={total_transactions}, {elapsed / 60:.1f}m"
        )
    ...
    log_path = log_dir / f"push_us_form4_{ts}.json"
    log_path.write_text(
        json.dumps(
            {
                ...
                "summary": {
                    ...
                    "filings_inserted": total_filings,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="从 data/form4/ 增量推送 Form 4 数据到 Supabase"
    )
    p.add_argument(
        "--tickers",
        type=str,
